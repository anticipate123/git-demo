<template>
  <div>
    <Row type="flex" justify="center" align="middle">
      ...
      <Col span="12">
        <Table :columns="powercolumns" :data="powerdata"></Table>
      </Col>
    </Row>
  </div>
</template>
...
export default {
    ...
    return{
      ...
      option : {
        ...
        radar: {
          ...
          indicator: [
            { name: '(2,5]', max: 60, color: '#333' },
            { name: '(5,10]', max: 60, color: '#333' },
            { name: '(10,15]', max: 60, color: '#333' },
            { name: '(15,20]', max: 60, color: '#333' },
            { name: '>20', max: 60, color: '#333' }
          ],
          splitArea: {
            areaStyle: {
        ...
        series: [
          {
            ...
            data: [
              {
                value: [3500, 5500, 1900, 1900, 6000],
                areaStyle: {
  ...
  methods : {
    getData(){
      this.$Spin.show();
      this.axios({
        url: "/api/statisticsAnalysis/signal/groupByPower/mongodb",
        method: "post",
        data: {
        ...
        for(let i=0;i<res.data.data.length;i++){
          ...
          this.powerdata[i].value = res.data.data[i].number;
        ...
        if( sum > 0 ){
          for(let i=0;i<res.data.data.length;i++){
            this.powerdata[i].percentage = (this.powerdata[i].value / sum * 100).toFixed(2) + '%';
          }
        }
    ...
    printChart(){
      this.myChart = this.echarts.init(this.$refs.powerChart, "default", {
